...
export function MarketplaceAwardCheckout() {
  ...
  const [availableMethods, setAvailableMethods] = useState<PaymentMethodKind[]>([]);
  ...
  const submit = async () => {
    if (!offer || !award || !selectedAddress || !selectedMethod) return;
    ...
    if (result.ok) {
      ...
      if (result.orderId) {
        window.location.assign(getMarketplaceCheckoutRoute(result.orderId));
        return;
      }
  ...
  useEffect(() => {
    if (!award) return;
    let active = true;
    void CommerceController.getSellerPaymentConfig(award.listing.sellerPubky)
      .then((config) => {
        if (!active) return;
        const methods = availablePaymentMethods(config);
        setAvailableMethods(methods);
        setSelectedMethod((current) => (current && methods.includes(current) ? current : (methods[0] ?? null)));
      })
      .catch(() => {
          ...
          <Card className="border">
            <CardContent className="grid gap-4 px-6">
              ...
              <Button asChild className="w-fit rounded-full">
                <Link href={getMarketplaceCheckoutRoute(orderId)} overrideDefaults>
                  Continue checkout
                </Link>
              </Button>
            </CardContent>
                ...
                <>
                  ...
                  <div className="grid gap-2">
                    ...
                    {availableMethods.length === 0 ? (
                      <Typography as="p" className="text-sm text-muted-foreground">
                        The seller has not set up a payment method yet, so Pay stays disabled.
                      </Typography>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {availableMethods.map((method) => (
                          ...
                          >
                            {method === 'bitcoin' ? '₿ Bitcoin' : method === 'stripe' ? 'Card (Stripe)' : 'PayPal'}
                          </Button>
                        ))}
                      </div>
                    )}
                  ...
                  <Button
                    ...
                    disabled={checkout.isSubmitting || !selectedMethod}
                    onClick={() => void submit()}
                    data-testid="marketplace-award-checkout-pay"
                  >
                    {checkout.isSubmitting ? 'Paying' : 'Pay'}
                  </Button>
                </>
              ) : (
