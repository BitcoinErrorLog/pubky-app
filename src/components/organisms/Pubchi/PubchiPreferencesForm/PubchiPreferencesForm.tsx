...
export function PubchiPreferencesForm() {
  ...
  return (
    <Card data-surface={PUBCHI_PREFERENCES_SURFACE} data-testid={PUBCHI_PREFERENCES_SURFACE} className="border border-border">
      <CardHeader>
      ...
      <CardContent>
        ...
        >
          ...
          <SwitchField control={form.control} name="include_disagreement" label="Include disagreement" disabled={loading} />
          ...
          <TopicField
            ...
            values={topics.map((topic, index) => (
              <span key={topic.label} className="flex items-center gap-1 rounded-full border border-border px-2 py-1 text-sm">
                {topic.label}
                <select
          ...
          <SwitchField control={form.control} name="proactive_enabled" label="Enable proactive suggestions" disabled={loading} />
          <SelectField control={form.control} name="max_suggestions_per_day" label="Maximum suggestions per day" options={Array.from({ length: 11 }, (_, index) => index)} />
          <SelectField control={form.control} name="quiet_hours_start" label="Quiet hours start (UTC)" options={Array.from({ length: 24 }, (_, index) => index)} />
          <SelectField control={form.control} name="quiet_hours_end" label="Quiet hours end (UTC)" options={Array.from({ length: 24 }, (_, index) => index)} />
          <SwitchField
            control={form.control}
