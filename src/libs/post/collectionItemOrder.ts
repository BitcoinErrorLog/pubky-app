...
/**
 * Release-shell compatibility: sort stream ids by raw collection item URIs.
 * The upstream UI stage moves callers to `collectionItemsToPostIds` plus
 * `sortPostIdsByMembership`.
 */
export function sortPostIdsByCollectionOrder(postIds: string[], envelopeItems: string[] | undefined): string[] {
  ...
  for (const postId of postIds) {
    (orderByPostId.has(postId) ? inCollection : rest).push(postId);
  }
  inCollection.sort((a, b) => (orderByPostId.get(a) ?? 0) - (orderByPostId.get(b) ?? 0));
