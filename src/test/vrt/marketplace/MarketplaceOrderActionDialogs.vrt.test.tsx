...
describe('Marketplace order action dialogs — visual regression', () => {
  it('renders the open return request dialog at desktop viewport', async () => {
    ...
    const screen = await renderForVRT(
      <ActionsHarness>
        <MarketplaceOrderActions order={deliveredOrder} isBuyer canCancel actOnOrder={async () => false} />
      </ActionsHarness>,
  ...
  it('renders the open shipment tracking dialog at desktop viewport', async () => {
    ...
    const screen = await renderForVRT(
      <ActionsHarness>
        <MarketplaceOrderActions order={paidOrder} isBuyer={false} canCancel actOnOrder={async () => false} />
      </ActionsHarness>,
      { viewport: VRT_VIEWPORT_DESKTOP },
    );
  ...
  it('renders the open external refund evidence dialog at desktop viewport', async () => {
    ...
    const screen = await renderForVRT(
      <ActionsHarness>
        <MarketplaceOrderActions order={returnReceivedOrder} isBuyer={false} canCancel actOnOrder={async () => false} />
      </ActionsHarness>,
      { viewport: VRT_VIEWPORT_DESKTOP },
    );
