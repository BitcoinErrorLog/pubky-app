...
describe('createMarketplaceListingSchema', () => {
  ...
  it('allows free shipping without a shipping price, but never a zero price', () => {
    ...
    expect(
      createMarketplaceListingSchema.safeParse({ ...base, freeShipping: true, shippingPrice: '' }).success,
    ).toBe(true);
    ...
    expect(
      createMarketplaceListingSchema.safeParse({ ...base, freeShipping: false, shippingPrice: '0' }).success,
    ).toBe(false);
    expect(
