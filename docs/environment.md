...
- **Optional/defaulted public values**: operational polling and TTL settings, moderated tags, exchange-rate API, Prelude, the Google Places Autocomplete browser key (`PUBKY_RUNTIME_GOOGLE_PLACES_API_KEY`; absent disables US address suggestions), Plausible, metadata/branding defaults, and external links. Missing values use the defaults in `src/libs/runtime-config/runtime-config.schema.ts`; malformed provided values still fail loudly.
