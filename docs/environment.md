...
- **Optional/defaulted public values**: operational polling and TTL settings, moderated tags, exchange-rate API, Prelude, Plausible, metadata/branding defaults, external links, and Pubchi (`PUBKY_RUNTIME_PUBCHI_ENABLED` default `false`; `PUBKY_RUNTIME_PUBCHI_API_URL` default empty hides the chat panel). Missing values use the defaults in `src/libs/runtime-config/runtime-config.schema.ts`; malformed provided values still fail loudly.
