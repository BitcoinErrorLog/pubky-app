...
- **Optional Bitcoin network (`bitcoinNetwork` — `PUBKY_RUNTIME_BITCOIN_NETWORK`)**: `mainnet` | `testnet` | `regtest`. Declares which network the payment rails settle on and gates the watch-only xpub claim: unset or unrecognised, the claim is refused with the named reason `bitcoin_network_unconfigured` (fail-closed at use, like the `locks-paykit` URL gate — the whole config parse does NOT fail, so the named reason can surface). It is never derived from `PUBKY_RUNTIME_ENV` or `testnet`.
- **Optional paykit setup URL (`paykitSetupUrl` — `PUBKY_RUNTIME_PAYKIT_SETUP_URL`)**: must be an `https://` URL; `http:` is accepted only for loopback hosts (`localhost`, `127.0.0.1`, `[::1]` — the default is `http://localhost:3102/setup` for local development). A non-HTTPS non-loopback value fails config parsing at boot. This is the same shared predicate (`isSecurePaykitOrigin` in `src/core/services/marketplace/paykit-origin.ts`) the token-carrying claim choke point and the seller setup-navigation builder enforce at use time — claim credentials and Ring approvals must never travel to a cleartext origin.
