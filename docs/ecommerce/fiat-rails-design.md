...
Paykit, Locks, and Pubky bridge *any* settlement system, not just blockchains. A fiat
...
*only* criterion with lock logic referencing exactly it (`lock_policy.rs:157-186`) — fine
...
The Lock Server talks to *one* payment backend over a small signed HTTP contract:
...
- **The overlay that wires them:** `payments-env/overlay/locks-server-entrypoint-paykit.sh`
  ...
  `[paykit] server_url` (`:110-112`). Trust is exactly: *one pinned ed25519 key, each way
  implicit* — the Lock Server trusts whatever the configured URL answers, and the
...
confirmation rule; there is no failure webhook — you *withhold* satisfaction and the
...
`marketplace-service/crates/service/src/locks.rs` is explicit: *"Verification is a pure
function of what Locks reports: nothing in this module accepts a client-supplied
status"* (`:20-21`). The only upstream call is `POST /verification-task-lookups` with
...
For the Bitcoin rail the payment *instruction* reaches the buyer out-of-band: Paykit
...
- The app never authors content locks: the listing studio explicitly *cannot* create a
...
Fiat rails actually fit the existing listing pricing *better* than Bitcoin does: a USD
...
*as the dependency for shipping*; written up as a ready-to-send proposal instead
...
3. The status contract is three states + confirmations + amount_matched (§1.5). Fiat
   maps cleanly: *undetected* = no completed checkout; *detected* = processor reports
   paid, settlement delay running; *confirmed* = paid AND the anti-chargeback delay
...
fiat verifier under a label that names a different system. It is *not* a security
...
| `POST /transactions/status` `{creator, bundle_id}` | same | `BTC` → forward. Fiat → answer from local state, which is *only* advanced by webhook-then-API-pull verification (§3.3). Unknown handle → 404 (stays `VerificationPending` upstream, `paykit_http_client.rs:447-470` test shows 404 is retryable). |
...
identity exposure is to the *processor* (unavoidable, disclosed — §4).
...
3. **`verify_payment(processor_ref) → Paid{amount_minor, currency} | Unpaid | Reversed` — the API pull.**
   The webhook only *schedules* this pull; state transitions happen exclusively on
...
| What Locks itself trusts | Identical in every case: the Lock Server operator and its configured verifier backend (§1.4). Locks is rail-agnostic; the *rails* differ. |
...
2. **After completion — marketplace dispute linkage.** The marketplace already has a
   ...
   marketplace dispute flow — *"buyer remedies leave the order disputed awaiting the
   external refund"* (`commands.rs:458`) — which already assumes refunds are external.
...
3. **The seller carries chargeback risk, and the UI says so.** With seller-owned
   ...
   state: *"Card and PayPal payments can be reversed by the buyer's bank for ~120–180
...
- Seller connects an existing or new Stripe account through Stripe-hosted Connect
  ...
  creates Checkout Sessions *on the connected account* (direct charges).
...
- Effort: **M–L** (PayPal multiparty requires a platform application review even for
  ...
  (operator-held, staging only, clearly marked — this is *not* the production shape).
...
- **Pay with card (Stripe)** — creates the same proof bundle + registration, then calls
  `/checkout-sessions` and opens Stripe Checkout. Copy: *"Card checkout is processed by
  ...
  delivery of high-value items until the settlement window passes."*
...
lock by wallet. Dual-rail listings (buyer picks BTC *or* USD at checkout) require
...
6. **Fiat verifier availability**: if it is down, `/invoices` fails and buyers can't
   *start* fiat payments (same failure mode as Paykit Server being down for Bitcoin —
...
- PayPal: Orders v2 API + lifecycle (capture status COMPLETED before fulfillment),
  ...
  (PAYMENT.CAPTURE.*, CUSTOMER.DISPUTE.*), dispute webhook sandbox testing. Accessed 2026-08.
