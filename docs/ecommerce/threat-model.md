...
| AUTH-06 | Account switch leaks Dexie projections        | Private data disclosure                  | Origin-scoped Dexie (`franky`); identity-keyed wipe on sign-out/switch under the auth-finalization lock (no-op if a different identity now owns the store); not per-pubky DB names |
