...
2. Encode it with the **same alphabet `export()` uses**. The d.ts calls that string “base64” (`pubky.d.ts:1099–1120`); whether the implementation is standard base64 or base64url is **not** settled by reading this repo. A mismatch is exactly what only the §9 step 0 staging proof can settle.
...
**Live confirmation does not block this design; it blocks shipping.** Section 9 step 0 is the gate. If the assumption is false, the user approves in Ring, Shop spins, then sees “sign-in failed” on the **primary** sign-in path — not a later commerce reconnect. The fallback is the minimal `Session.fromAuthToken` export in §8. Do not guess that failure from this repo.
...
- Handle: `Session.restore` from the `export()` alphabet applied to `SessionInfo` body (`pubky.d.ts:1099–1120`; `session/core.rs:220–256`). Alphabet (base64 vs base64url) is a §9 step 0 proof, not a source-reading claim.

**If** live staging shows `Session.restore` failing after a 2xx POST (cookie not stored, body ≠ `SessionInfo` serialize, or alphabet mismatch), request this **minimal** upstream export — nothing larger — as the **fallback** when step 0 fails:
...
**Step 0 (launch-blocking; design may proceed, shipping may not).** On staging, with a **real Ring approval** (not a mocked `AuthToken`):

1. `Client.fetch` POST of `token.toBytes()` to `https://_pubky.${z32}/session` returns 2xx.
2. A homeserver cookie is set (subsequent request carries it; HttpOnly so confirm via authenticated call, not `document.cookie`).
3. A subsequent authenticated request (`session.storage` write under `/pub/pubky.app`, or `GET /session`) succeeds **without another Ring approval**.
4. `Session.restore` of the POST body encoded with the `export()` alphabet succeeds. This is the only proof that restore's base64 matches `export()` (P3-2). Log status and restore success **without** token bytes or cookie values.
5. Record the **homeserver replica count** and whether routing is sticky. If more than one replica without sticky routing, the in-memory `seen` set is not cluster-wide — treat that as a known residual (P2-3), not as a silent assumption.

**If step 0 fails:** the user experience is: approve in Ring, Shop spins, then “sign-in failed” on the **primary sign-in path**. Stop. Use the §8 `Session.fromAuthToken` fallback; do not ship the dual POST.
...
- Manual / harness: direct Shop sign-in on staging, then open Orders — **zero** second QR. Bridged from pubky.app, then Orders — **exactly one** QR. Negative: after HS 2xx, tamper marketplace body → 401, reconnect appears. Step 0 must PASS before any of this ships.
