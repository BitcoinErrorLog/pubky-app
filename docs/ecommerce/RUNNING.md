...
- **Reads come from the service's role-scoped projections** (listings, offers, orders with embedded payment/shipment/return/dispute/refund/review sub-objects, payments, receipts, notifications, reports, the moderator dispute queue, and the scoped dispute case file). The interactive shopping UI sources every `expected_revision` from a fresh projection read and treats `REVISION_CONFLICT` as refetch-and-retry. Establishing the session still requires a signer approval whose in-app UX is an open product decision (see `service-auth.md`), so in a browser the durable-mode screens show the session requirement until a session is established programmatically; the integration suite below exercises the transport end to end.
