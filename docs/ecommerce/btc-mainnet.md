...
| 9 | **Staging cutover** | Railway `marketplace-service` in `pubky-marketplace-staging`: `PAYKIT_SERVER_URL=http://paykit-server-mainnet.railway.internal:3001`, then `railway redeploy -s marketplace-service` (variable changes alone do not restart paykit-server — `HANDOFF.md`, Known Decisions). Vercel `pubky-marketplace-staging`: `PUBKY_RUNTIME_PAYKIT_SETUP_URL` → the mainnet service's public URL; redeploy. | Set both back to the regtest service URLs and redeploy. Two variables, two redeploys, no data migration. | Medium. Staging sellers must re-claim; staging orders in flight against the regtest service stall and expire at the hold window (3600 s). Announce before, not after. |
