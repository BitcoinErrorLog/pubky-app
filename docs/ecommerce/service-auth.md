...
**The tradeoff, stated plainly.** This is a smaller widening than the marketplace token's: page-origin code that reads `sessionStorage` learns only that a messaging session existed and for which account — the credential itself stays in an HTTP-only cookie it cannot read. The real change is behavioral: the session now survives a same-tab reload without re-approval, so a user who walks away from an unlocked machine has a live messaging session for as long as the homeserver honors the cookie, where before a reload would have killed it. Same-tab-only is a deliberate choice of the narrowest storage that fixes the reload complaint: `sessionStorage` is per-tab, so a new tab, a new browser session, or another device still requires a fresh Ring approval — and the UI copy says so only where that is accurate.
