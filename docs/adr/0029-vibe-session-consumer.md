# ADR 0029: Vibe Session Consumer
...
Approved vibes are pubky-app forks served from `<slug>.vibes.pubky.app` (and same-site marketplace at `shop.pubky.app`). They are same-site with `pubky.app` and the homeserver, so the browser already attaches the homeserver `HttpOnly` cookie. Those forks still need the public `sessionExport` (`session.export()` — base64 `SessionInfo`: pubky + capabilities) to call `restoreSession` without a second grant.
...
A cached `#s=` export is further bounded twice so it cannot be reused as a stale hand-off. First, the 60-second TTL (`FRAGMENT_SESSION_EXPORT_TTL_MS` in `fragment.ts`) applies **only at decision time**: `hasPendingFragmentSessionExport` — the predicate that gates whether a restore starts — treats an expired capture as absent and drops it. Once a restore of this page load has begun, `takeFragmentSessionExport` applies the cached export **regardless of age**: the fragment leg runs only after the persisted-export leg exhausts its retries (10 attempts × 3 s sleeps plus request time), so on a slow network more than 60 seconds can pass between capture and take, and re-checking the TTL there would expire a legitimate same-page-load hand-off. Second, the cache is discarded once the first restore decision of the page load has run, whichever leg decided it (persist, fragment, or bridge — `discardFragmentSessionExport` in the `restorePersistedSession` `finally`); that discard is the hard reuse bound. After a later same-tab logout, `hasPendingFragmentSessionExport` therefore reads false and the hours-old fragment is never applied.
