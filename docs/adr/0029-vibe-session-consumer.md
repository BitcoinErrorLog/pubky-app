...
A cached `#s=` export is further bounded twice so it cannot be reused as a stale hand-off: it expires 60 seconds after capture (`FRAGMENT_SESSION_EXPORT_TTL_MS` in `fragment.ts`), and the cache is discarded once the first restore decision of the page load has run, whichever leg decided it (persist, fragment, or bridge — `discardFragmentSessionExport` in the `restorePersistedSession` `finally`). After a later same-tab logout, `hasPendingFragmentSessionExport` therefore reads false and the hours-old fragment is never applied.
